...
public class DriverManager {
  ...
  private Map<Integer, User> drivers = new HashMap<>();
   ...
   */
  public void createDriver(final User driver) {
  ...
  public void updateDriverAvailability(final int driverId, boolean newAvailability) {
    ...
    drivers.get(driverId).canOfferRide(newAvailability);
  }

  public List<User> getDrivers() {
    return drivers.values().stream().filter(User::isOfferingRide).collect(Collectors.toList());
  }
}